	...
	public class RampHitGenerator
	{
		...
		private List<HitObject> GenerateWallLineSeg(Vertex2D pv1, Vertex2D pv2, bool pv3Exists, float height1, float height2, float wallHeight, IItem item)
		{
			...
			if (height2 - height1 > 2.0 * PhysicsConstants.PhysSkin) { //!! use ballsize
				hitObjects.AddRange(GenerateWallLineSeg(pv1, (pv1 + pv2) * 0.5f, pv3Exists, height1, (height1 + height2) * 0.5f, wallHeight, item));
				hitObjects.AddRange(GenerateWallLineSeg((pv1 + pv2) * 0.5f, pv2, true, (height1 + height2) * 0.5f, height2, wallHeight, item));
